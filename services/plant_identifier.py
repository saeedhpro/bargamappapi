...
class PlantIdentifierService:
    ...
    async def identify_and_analyze(image_file: UploadFile, user: User):
        ...
        منطق جدید: ۱. گیاه توسط PlantNet شناسایی می‌شود. ۲. بررسی می‌شود که آیا **همین کاربر** قبلاً **همین گیاه** را
        در تاریخچه خود ثبت کرده است یا خیر. - اگر بله (revisited): رکورد جدیدی ساخته نمی‌شود. فقط عکس رکورد قبلی
        آپدیت شده و همان اطلاعات بازگردانده می‌شود. - اگر نه (new for user): - بررسی می‌شود آیا اطلاعات عمومی این
        گیاه در سیستم (توسط کاربر دیگری) کش شده است؟ - اگر بله: از اطلاعات کش‌شده استفاده می‌شود. - اگر نه: اطلاعات
        از سرویس GPT استعلام می‌شود. - در نهایت یک رکورد جدید **برای اولین بار** برای این کاربر و این گیاه در تاریخچه
        ثبت می‌شود.
        """
        ...
        if existing_user_history:
            ...
            details = existing_user_history.details or {}
            return {
                "status": "revisited",
        ...
        if cached_plant_info:
            ...
            try:
                prompt = (
                    ...
                    "لطفاً نام فارسی این گیاه را تعیین کرده و اطلاعات کامل نگهداری آن را ارائه بده.\n\n"
                    "قوانین نام‌گذاری فارسی:\n"
                    "1) اگر این گونه نام رسمی فارسی دارد → همان را بنویس.\n"
                    "2) اگر نام رسمی ندارد، از اطلاعات معتبر مبدأ یا محل رشد، یک نام غیررسمی بساز "
                    "(برای مثال گیاهان چینی → «... چینی»، ژاپنی → «... ژاپنی»، ایرانی → «... ایرانی»).\n"
                    "3) ترجمه ماشینی یا نام جعلی ممنوع است. اگر نام معتبر پیدا نکردی، مقدار باید رشتهٔ خالی باشد ("
                    "«»).\n\n"
                    "اکنون اطلاعات علمی، نگهداری و سلامت گیاه را در قالب فقط یک JSON معتبر با ساختار زیر بده:\n"
                    "{\n"
                    "  \"common_name_fa\": \"نام فارسی رسمی یا غیررسمی\",\n"
                    "  \"description\": \"یک توضیح جامع و دقیق شامل منشاء، ظاهر گیاه (برگ، گل، ساقه)، زمان گل‌دهی یا "
                    "رشد، کاربردها، و جزئیات کلی نگهداری — متن فارسی و حدود دوازده برابر نسخه کوتاه\",\n"
                    "  \"water\": \"خلاصه نیاز آبی\",\n"
                    "  \"water_detail\": \"توضیح کامل دفعات و نحوه آبیاری\",\n"
                    "  \"light\": \"نوع و شدت نور مورد نیاز\",\n"
                    "  \"light_detail\": \"توضیح کامل دربارهٔ محل مناسب نور و تأثیر نور بر رشد گیاه\",\n"
                    "  \"temp\": \"دمای مناسب\",\n"
                    "  \"fertilizer\": \"نوع کود و تناوب مصرف\",\n"
                    "  \"difficulty\": \"درجه سختی نگهداری\",\n"
                    "  \"toxicity\": \"آیا سمی است یا خیر و برای چه موجوداتی خطر دارد\",\n"
                    "  \"diseases\": \"آفات و بیماری‌های رایج با توضیح کوتاه هرکدام\",\n"
                    "  \"pest_control\": \"روش‌های مقابله، پیشگیری و درمان آفات و بیماری‌ها (زیر نظر متخصص یا "
                    "طبیعی)\"\n"
                    "}\n"
                    "تمام مقادیر باید کاملاً به فارسی برگردانده شوند."
                )
                ...
                chat_response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": (
                            "تو یک گیاه‌پزشک متخصص و استاد اکولوژی هستی. "
                            "پاسخ باید فقط یک JSON معتبر باشد که شامل تمام جزئیات مربوط به نگهداری، "
                            "آفات، بیماری‌ها و مبارزه با آن‌ها به زبان فارسی است."
                        )},
        ...
        response = {
            **(care_info or {}),
            ...
            "in_garden": False,
            "diseases": care_info.get("diseases", ""),
            "pest_control": care_info.get("pest_control", ""),
        }
        return response
    ...
    async def identify_and_analyze_new(image_file: UploadFile, user: User):
        ...
        try:
            prompt = (
                ...
                "\"common_name_fa\": \"نام فارسی اگر موجود بود اگه نبود ببین چه اسم غیررسمی فارسی براش هست. حتما این "
                "گزینه پر بشه و خالی نده\",\n"
